...
function perceptron_setup(data,labels)
    ...
    function f(x)
        β = x[1:end-1];
        β0 = x[end];
        return inv(m) * sum(max(0, 0 - labels[i]*(data[i,:]'*β-β0)) for i=1:length(labels))
    end

    function ∇f(x)
        β = x[1:end-1];
        β0 = x[end];
        v = Int16.(0 .<= 0 .- labels.*(data*β .- β0));
        grad_β = inv(m) * sum(-v[i]*labels[i]*data[i,:] for i=1:length(labels))
...
function perceptron_compute_optval(data,labels)
    ...
    problem = minimize((1/m) * sum(max(0, 0 - Diagonal(labels)*(data*β - β0*ones(m)) )))
...
function perceptron_plot(data,labels,x_opt,x_num)
    ...
    yy_opt = -(x_opt[1]*xx_axis .- x_opt[end])./x_opt[2]
    yy = -(x_num[1,end]*xx_axis .- x_num[end,end])./x_num[2,end]
